...
public class Meduse {
	...
	Meduse(int n) {
		...
		for (int i = 1; i < n; i++) {
			...
			balls.add(new Ball(this, balls.get(i - 1).position.x + newPos.x, balls.get(i - 1).position.y + newPos.y, 0,
					0, 1));
		}
		for (int i = 0; i < n - 2; i++) {
			linkList.add(new Link(this, i, i + 1, i + 2));
		}
		for (int i = 0; i < n - 2; i++) {
			muscleList.add(new Muscle(this, i, i + 1, i + 2, 1f * R, 1.9f * R, rnd.nextFloat(), rnd.nextFloat(),
	...
	void calculateForcesOfLinks() {
		...
		for (int ballIndex = 0; ballIndex < n; ballIndex++) {
			...
			// DODAWANIE SI� ODDZIA�YWANIA MI�DZY KULKAMI aka SI�
			// SPRʯYSTO�CI LINK�W
			if (ballIndex > 0) {
	...
	/**
	 * Oblicza siły oddziaływania ruchu poączeń z wodą - siłę napędową
	 * izapisuje do
	 */
	void calculateForcesOfWater() {
		...
		// Sila wody to wektor skierowany prostopadle do miesnia zalezny od
		// zmiany pola trojkata IJK dla danej trojki
		/*
		 * for(int i=0;i<n-2;i++){ muscleList.get(i).calculateWaterImpact(); }
		 */
	}
	...
	static void CrossMeduses(Meduse man, Meduse woman) {
		...
		for (int ii = 0; ii < man.muscleList.size(); ii++) {
			...
			tmpValMen = crossValues(man.muscleList.get(ii).contractLength, woman.muscleList.get(ii).contractLength,
					manQuotion, womanQuotion);
			tmpValWoman = crossValues(man.muscleList.get(ii).contractLength, woman.muscleList.get(ii).contractLength,
					womanQuotion, manQuotion);
			man.muscleList.get(ii).contractLength = tmpValMen;
			woman.muscleList.get(ii).contractLength = tmpValWoman;

			tmpValMen = crossValues(man.muscleList.get(ii).retracrLength, woman.muscleList.get(ii).retracrLength,
					manQuotion, womanQuotion);
			tmpValWoman = crossValues(man.muscleList.get(ii).retracrLength, woman.muscleList.get(ii).retracrLength,
					womanQuotion, manQuotion);
			man.muscleList.get(ii).retracrLength = tmpValMen;
			woman.muscleList.get(ii).retracrLength = tmpValWoman;

			tmpValMen = crossValues(man.muscleList.get(ii).contractTime, woman.muscleList.get(ii).contractTime,
					manQuotion, womanQuotion);
			tmpValWoman = crossValues(man.muscleList.get(ii).contractTime, woman.muscleList.get(ii).contractTime,
					womanQuotion, manQuotion);
			man.muscleList.get(ii).contractTime = tmpValMen;
			woman.muscleList.get(ii).contractTime = tmpValWoman;

			tmpValMen = crossValues(man.muscleList.get(ii).retractTime, woman.muscleList.get(ii).retractTime,
					manQuotion, womanQuotion);
			tmpValWoman = crossValues(man.muscleList.get(ii).retractTime, woman.muscleList.get(ii).retractTime,
					womanQuotion, manQuotion);
			man.muscleList.get(ii).retractTime = tmpValMen;
			woman.muscleList.get(ii).retractTime = tmpValWoman;

			tmpStateMan = crossStates(man.muscleList.get(ii).rightTwisted, woman.muscleList.get(ii).rightTwisted,
					manQuotion);
			tmpStateWoman = crossStates(man.muscleList.get(ii).rightTwisted, woman.muscleList.get(ii).rightTwisted,
					womanQuotion);
