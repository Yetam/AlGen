...
public class Muscle {
	...
	float field;
	float newfield;
	Vec field_rji = new Vec();
	Vec field_rjk = new Vec();
	Vec ortogonal2rik = new Vec();
	...
	void calculateWaterImpact(){
		//Obliczanie zmiany pola pomiedzy ramionami
		field_rji.subInto(meduse.balls.get(startBall).position, meduse.balls.get(middBall).position);
		field_rjk.subInto(meduse.balls.get(endBall).position, meduse.balls.get(middBall).position);
		newfield = Math.abs(field_rji.x*field_rjk.y - field_rjk.x*field_rji.y);
		float difference = (field - newfield)*(field - newfield);
		field = newfield;
		
		//obliczanie wektora orto do miesnia ktory niejako bedzie sila napedowa
		ortogonal2rik.subInto(meduse.balls.get(endBall).position, meduse.balls.get(startBall).position);	//obliczanie długości mięśnia
		ortogonal2rik.rotateNegPi2();
		ortogonal2rik.scaleToUnit();
		if(!rightTwisted){
			ortogonal2rik.scale(-1f);
		}
		//dodawanie tego syfu z gory do sil
		ortogonal2rik.scale(difference);
		meduse.balls.get(startBall).linkForce.add(ortogonal2rik);
		meduse.balls.get(middBall).linkForce.add(ortogonal2rik);
		meduse.balls.get(startBall).linkForce.add(ortogonal2rik);
	}
